...
#include "TreeBRLPolicyActions.h"
...
TreeBRLPolicyActions::TreeBRLPolicyActions(std::shared_ptr<DiscreteEnvironment> environment_,
				int n_states_,
                 int n_actions_,
                 real gamma_,
				 MDPModel* belief_,
...
environment = environment_;
    ...
    switch(algorithm) {
	case PLC:  Qs(n_policies*n_actions); break;
...
TreeBRLPolicyActions::~TreeBRLPolicyActions()
{
...
void TreeBRLPolicyActions::Reset()
{
    current_state = -1;
    current_action = -1;
}

void TreeBRLPolicyActions::Reset(int state)
{
    current_state = state;
    current_action = -1;
}
...
/// Full observation
real TreeBRLPolicyActions::Observe (int state, int action, real reward, int next_state, int next_action)
{
    if (state>=0) {
...
/// Partial observation 
real TreeBRLPolicyActions::Observe (real reward, int next_state, int next_action)
{
...
int TreeBRLPolicyActions::Act(real reward, int next_state)
{
...
//printf("adding transtion %d %d %f %d and deleting root policy\n",current_state, current_action, reward, next_state);
	...
	logmsg("Check the calling of SPARSE method in Act");
    ...
    for (int j=0; j<root_policies.size(); ++j) {
    ...
    if (algorithm == PLC && root_policies.size()){
    	for (int i=0; i<root_policies.size(); ++i) {
		delete root_policies[i];
    	}
    	root_policies.clear();
    }
...
TreeBRLPolicyActions::BeliefState TreeBRLPolicyActions::CalculateSparseBeliefTree(int n_samples, int n_TS)
{
...
TreeBRLPolicyActions::BeliefState TreeBRLPolicyActions::CalculateSparserBeliefTree(int n_samples, int K_step, int n_TS)
{
    // Initialise the root belief state
    BeliefState belief_state(*this, belief, current_state);
	...
	case PLC: belief_state.SparserExpandAllActions(n_samples,n_policies,K_step); buffer = n_policies*n_actions; break;
...
TreeBRLPolicyActions::BeliefState TreeBRLPolicyActions::CalculateBeliefTree()
{
    // Initialise the root belief state
    BeliefState belief_state(*this, belief, current_state);
...
TreeBRLPolicyActions::BeliefState::BeliefState(TreeBRLPolicyActions& tree_,
...
TreeBRLPolicyActions::BeliefState::~BeliefState()
{
	tree.size--;
...
void TreeBRLPolicyActions::BeliefState::SparserExpandAllActions(int n_samples,int n_policies, int K_step)
{
    if (t >= tree.horizon) {
        return;
    }
	...
	myset.insert(ArgMax(policies[i]->getActionProbabilities(state)));
	delete PI_objects[i];
    }

	//Generating policies with remaining unseen first actions
	for(int i=0; i<tree.n_actions; ++i){
		if ( myset.find(i) == myset.end() ) {
			Vector prob(tree.n_actions);
			prob[i] = 1.0;
			for (int j=0; j<n_policies; ++j) {
				FixedDiscretePolicy* newpol = new FixedDiscretePolicy(tree.n_states,tree.n_actions,policies[j]->p);
				newpol->p[state] = prob;
				policies.push_back(newpol);
			}
		}
	}
	...
	real p = 1.0 / (1.0*n_samples*n_policies + policies.size()-n_policies);
...
//		    real r = belief_clone->GenerateReward(next_state,next_action);
    ...
    for (uint i=0; i<children.size(); ++i) {
        children[i]->SparserExpandAllActions(tree.n_samples,n_policies,K_step);
    }
}
...
void TreeBRLPolicyActions::BeliefState::SparserAverageExpandAllActions(int n_samples,int n_policies, int K_step)
{
    if (t >= tree.horizon) {
        return;
    }
...
void TreeBRLPolicyActions::BeliefState::SparserRandomExpandAllActions(int n_samples,int n_policies,int K_step)
{
    if (t >= tree.horizon) {
        return;
    }
...
///
void TreeBRLPolicyActions::BeliefState::SparseExpandAllActions(int n_samples)
{
    if (t >= tree.horizon) {
        return;
    }
...
void TreeBRLPolicyActions::BeliefState::ExpandAllActions()
{
    if (t >= tree.horizon) {
        return;
    }
...
real TreeBRLPolicyActions::BeliefState::CalculateValues(LeafNodeValue leaf_node, int buffer)
{
...
real TreeBRLPolicyActions::BeliefState::UTSValue()
{
...
real TreeBRLPolicyActions::BeliefState::LTSValue()
{
    real discount = tree.gamma;
	int n_samples = 2;
...
real TreeBRLPolicyActions::BeliefState::MeanMDPValue()
{
    real discount = tree.gamma;
	const DiscreteMDP* model = belief->getMeanMDP();
	ValueIteration VI(model, discount);
