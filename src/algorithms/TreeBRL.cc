...
int TreeBRL::Act(real reward, int next_state)
{
    ...
    BeliefState belief_state = CalculateSparserBeliefTree(5 , 3 , 1);
...
TreeBRL::BeliefState TreeBRL::CalculateSparserBeliefTree(int n_samples,int K_step, int n_TS)
{
    // Initialise the root belief state
    BeliefState belief_state(*this, belief, current_state);
    belief_state.SparserExpandAllActions(n_samples,K_step);
    belief_state.CalculateValues(leaf_node_expansion);
    //belief_state.CalculateLowerBoundValues(n_TS),
    //belief_state.CalculateUpperBoundValues(n_TS));
    return belief_state;
...
TreeBRL::BeliefState::BeliefState(TreeBRL& tree_,
	...
	: tree(tree_), 
	  belief(belief_),
	  state(state_),
	  prev_reward(prev_total_r), probability(1), prev(prev_), t(prev_->t + 1)
{
...
void TreeBRL::BeliefState::SparserExpandAllActions(int n_samples,int K_step)
{
    ...
    real V_next = 0;
    DiscreteMDP* mean_mdp = belief->getMeanMDP();
	...
	children.push_back(new BeliefState(tree, belief_clone, next_state, total_reward, this));
	delete model;
    }
