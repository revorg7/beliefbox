...
//	std::cout << std::string(buffer,length);
	MDPModel *obj = reinterpret_cast<MDPModel *>(buffer); 	//both type of casting is possible, i.e MDPModel and DiscreteMDPCountsSparse
//	MDPModel *obj1 = reinterpret_cast<DiscreteMDPCountsSparse *>(buffer);
	DiscreteMDPCountsSparse *obj1 = reinterpret_cast<DiscreteMDPCountsSparse *>(buffer);
//printf("ns:%d\n",obj->getNStates());
obj->ShowModelStatistics();
obj1->getDirichletParameters(1,0).print(stdout);printf("\n");

	for (int i=0; i<4;i++ ){
		Vector v = obj->getTransitionProbabilities(i,0);
		v.printf(stdout);
		printf("p(r=0):%f p(r=1):%f\n",obj->getRewardProbability(i,0,0),obj->getRewardProbability(i,0,1));
	}
