...
int main(int argc, char** argv) {
    ...
    int leaf_value = TreeBRLPolicyAvg::LeafNodeValue::NONE;
    int algorithm = TreeBRLPolicyAvg::WhichAlgo::PLCAVG;
    int n_experiments = 5;
...
//    printf("# full sampling\n");
	...
	// Adding for comparision to USAMPLING
            DiscreteMDPCounts* discrete_mdp =  new DiscreteMDPCounts(n_states, n_actions,
                                                  dirichlet_mass,
                                                  reward_prior);
            DiscreteMDPCounts* model= (DiscreteMDPCounts*) discrete_mdp;
    ...
    for (int experiment=0;
        ...
        TreeBRLPolicyAvg tree (n_states, n_actions, discounting, &belief, rng, planning_horizon, (TreeBRLPolicyAvg::LeafNodeValue) leaf_value,(TreeBRLPolicyAvg::WhichAlgo) algorithm);
        // Set state to 0
