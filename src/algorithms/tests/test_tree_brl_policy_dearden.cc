...
real RunExperiment(shared_ptr<Mazeworld> environment,
                   TreeBRLPolicy& tree, 
...
int main(int argc, char** argv) {
    ...
    shared_ptr<Mazeworld> environment;
...
{
	int x,y,next_state,encoded_state,encoded_next_state,width,steps;
	width = 7;
	static const int schedule[] = {16,3};
...
//printf("start state is %d\n",environment->getState());
    for (int t=0; t<n_steps; ++t) {
		if (t<500) steps = schedule[0]; else steps = schedule[1]; 
        int state = environment->getState();
		x = state % width;
		y = (state - x) / width;
		encoded_state = helper.compress(helper.compress_flag(environment->flags_position), x, y);
        int action = tree.Act(reward,encoded_state);
...
//        printf("adding this one %d %d %f %d # s a r next_s\n", state, action, reward,environment->getState());
		...
		x = next_state % width;
		y = (next_state - x) / width;
		encoded_next_state = helper.compress(helper.compress_flag(environment->flags_position), x, y);
        tree.belief->AddTransition(encoded_state, action, reward, encoded_next_state);

//printf("outside adding transition\n");
		action = ArgMax( tree.root_policy->getActionProbabilities(encoded_next_state) ) ;
        state = next_state;
