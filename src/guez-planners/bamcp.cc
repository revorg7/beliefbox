...
int BAMCP::SelectAction(uint current_state)
{
  ...
  step++;
#ifdef DEBUG1
printf("UCT finished, calling greedyUCB\n");
#endif
...
printf("a nxts is %d %d\n",action,observation);
    ...
    if (Params.Verbose >= 3)
    {
		...
		VNODE*& vnode = qnode.Child(observation);
#ifdef DEBUG1
    if (!vnode && !terminal) printf("new-obs doesnt exist\n");
#endif
    if (!vnode && !terminal && qnode.Value.GetCount() >= Params.ExpandCount){
#ifdef DEBUG1
		printf("Also action tried more than once\n");
#endif
