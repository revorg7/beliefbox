...
Mazeworld::Mazeworld(const char* fname,
                     real random_,
                     real pit_,
                     real goal_,
                     real step_)
    : total_time(0),flags_collected(0),
    random(random_), pit_value(pit_), goal_value(goal_), step_value(step_)
{
    ...
    //reward = 0.0;
    logmsg("Mazeworld, states: %d, random: %f, pit: %f, goal: %f, step: %f\n",
           n_states,
           random,
           pit_value,
           goal_value,
...
DiscreteMDP* Mazeworld::getMDP()  const
{
...
bool Mazeworld::Act(const int& action)
{
    int x = state % width;
    int y = (state - x) / width;
    assert(state == getState(x, y));
    assert(whatIs(x,y) != INVALID && whatIs(x, y) != WALL);
//printf("acting state is %d with x,y = %d %d\n",state,x,y);
//if (state ==2) printf("state is pit with %d\n",whatIs(x,y));

    if (whatIs(x,y) == PIT){
// printf("inside sate is %d\n",state);
	if(std::find(flags_position.begin(), flags_position.end(), state)==flags_position.end()){
	    flags_collected += 1;
	    flags_position.push_back(state);
//	printf("pit found\n");
//	printf("flag collected here\n");
	}
    }
...
//printf("ola terminal-state and reward is %d %f\n",terminal_state,flags_collected/3.0);
	reward = flags_collected/3.0;
        return false;
    }
    return true;
}
...
void Mazeworld::CalculateDimensions(const char* fname)
{
    std::ifstream ifs(fname, std::ifstream::in);
    if (!ifs.is_open()) {
        Serror ("Could not open file %s", fname);
